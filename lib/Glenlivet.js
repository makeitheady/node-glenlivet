...
var HttpBottleResponder = function (req, resp) {
    return function (err, bottleRequest, bottleResponse) {
        ...
        if (!err) {
            ...
            resp.status(bottleResponse.httpStatus || 200);
        } else {
            value = {
                error: err[0]? err[0].message: err.message
            };
        
            resp.status(bottleResponse.httpStatus || 500);
        }

        if (bottleResponse && bottleResponse.headers) {
            resp.set(bottleResponse.headers);
        };
...
var Glenlivet = {
    ...
    plugins: {
        ...
        sessionFromHeader: require('./plugins/sessionFromHeader')
    },
    ...
    init: function (options) {
        ...
        this._bottleOptions = options.bottleOptions || {};
        this._globalPlugins = options.globalPlugins || [];
    ...
    bottle: function (name, specs) {
        specs = _.extend({}, this._bottleOptions, specs);
        specs.plugins = specs.plugins || [];
        specs.name = name;

        if (this._globalPlugins) {
            specs.plugins = this._globalPlugins.concat(specs.plugins);
        }
    ...
    _createRESTEndpoint: function (bottle, specs) {
        ...
        app[specs.method.toLowerCase()](specs.path, function (req, resp) {
            var overrides = _.extend({}, req.query, req.body, getParams(req), {headers:req.headers}),
            ...
            bottle.serve(overrides, function (err, bottleRequest, bottleResponse) {
                ...
                req.params.property = bottle.options.rest.property || 'json';
